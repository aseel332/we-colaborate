...
export default function BranchCard({branch}) {
  ...
  return (
    <div className="border border-gray-300 rounded-2xl p-4 shadow-md hover:shadow-lg transition">
      ...
      <button className="bg-blue-600 text-white px-3 py-1 rounded-2xl hover:bg-blue-700 transition" onClick={() => {navigate(`/project/${projectId}/branch/${branch._id}`)
    localStorage.setItem("currentBranch", JSON.stringify(branch._id));
  }}>View</button>
    </div>
  );
}
